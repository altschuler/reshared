...
export const createUseEditor =
    <T>(defaultState: T, schema?: Joi.ObjectSchema<T>) =>
    (initial?: Partial<T>): EditorState<T> => {
        const [state, setState] = useState<T>(
            initial ? { ...defaultState, ...initial } : defaultState,
        );
        ...
        const update = useCallback(
            (update: Partial<T>) => setState({ ...state, ...update }),
            [state],
        );
        ...
        const reset = useCallback(
            (forceDefault?: boolean) =>
                resetTo(
                    forceDefault ? defaultState : { ...defaultState, ...initial } || defaultState,
                ),
            [initial, resetTo],
        );
        ...
        const errors = useMemo(() => {
            ...
            const result = schema.validate(debouncedState, {
                abortEarly: false,
                allowUnknown: true,
            });
